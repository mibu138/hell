...
static void
cmdListFn(Hell_Grimoire* grim, void* data)
{
...
static void
cmdEchoFn(Hell_Grimoire* grim, void* data)
{
...
static void
cmdSetVarFn(Hell_Grimoire* grim, void* data)
{
    ...
    hell_SetVar(grim, varname, val, 0);
}

static void
varListFn(Hell_Grimoire* grim, void* data)
{
    for (Var* var = grim->variables; var; var = var->next)
    {
        hell_Print("%s: %f\n", var->name, var->value);
...
static void
varSetFn(Hell_Grimoire* grim, void* data)
{
}
...
static bool
consoleEventHandler(const Hell_Event* event, void* pGrimoire)
{
    Hell_Grimoire* grim = (Hell_Grimoire*)pGrimoire;
    hell_AddNText(grim, event->data.conData.ptr, event->data.conData.ptrLen);
    hell_AddChar(grim, '\n');
    return true;
}

const char*
hell_GetArg(const Grim* grim, unsigned int i)
{
    if (i >= grim->cmdArgc)
...
int
hell_GetArgC(const Grim* grim)
{
    return grim->cmdArgc;
}

void
hell_AddCommand(Hell_Grimoire* grim, const char* cmdName, Hell_CmdFn function,
                void* data)
{
...
void
hell_RemoveCommand(Hell_Grimoire* grim, const char* cmdName)
{
    assert(strnlen(cmdName, MAX_CMD_NAME_LEN) < MAX_CMD_NAME_LEN);
    Cmd *cmd, *prev;
    cmd  = grim->commands;
...
void
hell_AddCommand2(Hell_Grimoire* grim, const char* cmdName, Hell_CmdFn function,
...
void
hell_SetVar(Hell_Grimoire* grim, const char* name, double value,
            const Hell_VarFlags flags)
{
...
Var*
hell_GetVar(Grim* grim, const char* name, double value,
            const Hell_VarFlags flags)
{
    Var* var = findVar(grim, name);
    if (var)
...
void
hell_AddText(Grim* grim, const char* text)
{
...
void
hell_AddNText(Grim* grim, const char* text, unsigned int l)
{
    if (grim->cmdTextCursor + l >= MAX_CMD_BUFFER)
    {
        hell_Error(0, "Cmd text buffer overflow");
...
void
hell_AddChar(Grim* grim, const char c)
{
...
void
hell_CreateGrimoire(Hell_EventQueue* queue, Grim* grim)
{
...
hell_Incantate(Grim* grim)
{
    ...
    while (grim->cmdTextCursor)
    {
        ...
        else
        {
            ...
            memmove(grim->cmdTextBuf, grim->cmdTextBuf + i,
                    grim->cmdTextCursor);
        }
...
void
hell_DestroyGrimoire(Grim* grim)
{
    memset(grim, 0, sizeof(Hell_Grimoire));
}

uint64_t
hell_SizeOfGrimoire(void)
{
    return sizeof(Hell_Grimoire);
}

Hell_Grimoire*
hell_AllocGrimoire(void)
{
